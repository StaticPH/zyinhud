...
public class ItemSelector extends ZyinHUDModBase {
    ...
    private static boolean AdjustSlot(int direction) {
        if (!mc.isSingleplayer()) {
            if (!currentInventory.get(currentHotbarSlot).isEmpty() && currentInventory.get(currentHotbarSlot).isEnchanted()) {
        ...
        for (int i = 0; i < 36; i++) {
            ...
            if (!mc.isSingleplayer()
                    && currentInventory.get(memory).isEnchanted())
                continue;
    ...
    private static void SelectItem() {
        ...
        if (!targetStack.isEmpty()) {
            if (!mc.isSingleplayer()) {
                if ((!currentStack.isEmpty() && currentStack.isEnchanted())
                        || targetStack.isEnchanted()) {
                    ZyinHUDRenderer.DisplayNotification(Localization.get("itemselector.error.enchant"));
                    Done();
                    return;
                }
